...
public:
    ...
    /** 获取图层名称。基于 OGRLayer::GetName()。 */
    std::string getName() const;

    /** 获取几何类型。基于 OGRLayer::GetGeomType()。 */
    ...
    /** 获取字段总数。基于 LayerDefn，不依赖当前要素。 */
    int getFieldCount() const;

    /** 按索引获取字段名。越界返回空字符串。 */
    std::string getFieldName(int index) const;

    /** 按索引获取字段类型。越界返回 OFTInteger。 */
    ...
    /** 是否支持属性过滤（GDAL 所有 OGRLayer 都支持）。 */
    bool supportsAttributeFilter() const;

    /** 是否支持快速空间过滤（有空间索引时返回 true）。 */
    bool supportsFastSpatialFilter() const;

    /** 是否支持快速要素计数（驱动内置计数时返回 true）。 */
    ...
    /** 获取字段类型的人类可读名称（如 "Integer"、"String"）。 */
...
public:
    ...
    /** 获取图层数量。基于 GDALDataset::GetLayerCount()。 */
    int getCount() const;

    /** 按索引获取图层视图。越界返回无效 GdbDataset。 */
    GdbDataset get(int index) const;

    /** 按名称获取图层视图。不存在返回无效 GdbDataset。 */
