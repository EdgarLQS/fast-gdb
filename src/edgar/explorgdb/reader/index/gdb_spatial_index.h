...
public:
     ...
     * @param xorig 图层 X 原点。
     * @param yorig 图层 Y 原点。
     * @param xyscale 图层 XY 缩放因子。
     * @param grid_resolutions 空间索引网格分辨率列表。
     * @param max_fid 最大 FID；为 0 时使用排序去重。
     * @param merge_x_ranges 是否合并完整 Y 范围的 X 查询。
     * @return 空间索引命中的候选 FID 列表。
     */
