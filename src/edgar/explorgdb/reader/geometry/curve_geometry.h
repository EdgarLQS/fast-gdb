...
struct CurveLinearizationResult {
    ...
    /** 判断曲线线性化结果是否可用。
     * @return 状态为 Valid 或 Empty 时返回 true。
     */
    bool valid() const {
 ...
 */
/** 将曲线描述符展开为线性部件。
 * @param request 曲线顶点、部件、描述符、坐标变换和资源限制。
 * @return 线性化结果；失败时包含状态和诊断信息，不返回部分几何。
 */
...
public:
    /** 拒绝所有曲线几何并返回 UnsupportedCurve 状态。
     * @param request 曲线几何构造请求。
     * @return 标记为不支持的几何模型。
     */
