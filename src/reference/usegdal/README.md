...
```text
usegdal/
├── core/        connection and error helpers
├── dataset/     datasource and dataset wrappers
├── feature/     feature, field, and recordset wrappers
├── query/       query construction and spatial relation helpers
└── transaction/ transaction and batch-write examples
```

The subdirectories describe the historical wrapper's concepts only; they do not
define supported fast-gdb modules or installed include paths.
