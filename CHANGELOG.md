...
- Dedicated real OpenFileGDB visibility tests proving that normal GDAL `SetFeature()` writes and GDAL transaction commits affect only `working_path()` until Store publication.
- Focused `fast_gdb_gdal_visibility_test_runner` and Ubuntu CI gate for old/new Reader visibility across GDAL write, GDAL close, Store publish, and snapshot refresh.
...
- GDAL `CommitTransaction()` is explicitly treated as a working-GDB edit commit; only Store `PublishedDurable` changes the generation visible to newly acquired Readers.
...
- Real OpenFileGDB write/read visibility tests and their focused CI target are implemented; formal conclusions still require successful workflow steps, logs and artifacts.
