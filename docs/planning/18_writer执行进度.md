...
- 原功能 PR：#11（Draft/Open，待确认树覆盖后关闭）
- 外部阻塞：Issue #12（Open）
...
- PR #11：Draft/Open，head=`codex/m18-1-macos-test-contract-ci`，base=`main`。
- 原远端分支：分支搜索未返回；PR 仍保留 head SHA `6b8d3d5565246be5cf306b4d95a4759455bdcbbe`，因此不把“已删除”外推为树已验证覆盖。
- Issue #12：Open。
...
6. PR #11 最终树覆盖确认后关闭；Issue #12 仅在 Actions 恢复并产出证据后关闭。
