...
v1 目标先锁定为**只读生产化**。v2 继续做只读生产适配，补齐旧记录兼容、General 几何、曲线、Raster 和 MultiPatch 边界；写入生产化顺延到 v3 或后续阶段，避免读取适配和写入元数据更新同时展开导致范围失控。
...
v3 或后续阶段再进入写入生产化，避免和 v2 只读适配互相阻塞。
...
| 写入生产化过早展开 | v1/v2 不处理写入系统表一致性，只保留 v3 或后续计划 |
