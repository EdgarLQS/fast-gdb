...
| [ADR-009：统一 FileGDB 访问与 GDAL/S3 路由](ADR-009-unified-filegdb-routing.md) | Proposed | 统一只读访问 seam；本地优先 fast-gdb，S3/能力缺口由 GDAL OpenFileGDB 处理 |
...
fast-gdb
  ...
  ├─ Proposed unified Dataset/Layer/Cursor routing
  └─ no Writer product

GDAL/OpenFileGDB
  ├─ all FileGDB creation and editing
  ├─ Proposed S3/object-storage read adapter
...
8. ADR-009 的统一入口、S3 路由和 `FastFileGDB` 驱动在实现和真实环境验收前不得写成已支持能力。
