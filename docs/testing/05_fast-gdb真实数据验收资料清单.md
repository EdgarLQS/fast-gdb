# 05 — fast-gdb 真实数据验收资料清单

**文档状态**：当前真实数据验收规范
**更新日期**：2026-07-13
**适用范围**：`fast_gdb_linear`、`fast_gdb_hybrid` Reader、统一几何模型、WKB/WKT 输出、空间查询和 GDAL 回退链路
**验收目标**：后续新增能力和新增真实数据按本清单准备，避免只验证“能打开文件”，却没有足够证据判断几何语义、FID 映射和查询结果是否正确。
...
4. 真实验收分为“数据覆盖完成”和“逐要素对照完成”；当前版本结论以最终验收报告为准。

## 2. 必需数据清单

下表中的“必需”是后续新增能力或扩大支持范围时应具备的数据类别；已有样本的状态按当前验收报告解释。
...
当前已有 `testcurve.gdb` 和 `参数化数据_liqs.gdb`，支持范围内的曲线、Z/M/ZM、FID、坏拓扑、CRS
和性能证据已经形成。后续优先补充的是新增能力证据，而不是重复制作同类曲线数据：

1. 35GB/5 亿级真实数据的读取、过滤重写、追加和索引构建性能仍需单独实测；
2. MultiPatch 完整 part type/表面拓扑如需支持，另立实现任务；
3. 新增 FileGDB 描述符或新增输出语义时，补充对应原生样本和逐要素对照。

当前支持范围内的发布结论见
[最终等价与发布验收报告](../evidence/13_fast-gdb最终等价与发布验收报告.md)；新增范围必须重新完成本清单对应的证据和对照。
