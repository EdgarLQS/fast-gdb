...
**当前结论**：fast-gdb 的纯 C++ Reader、统一几何模型、索引查询和主要自动化测试已经形成可复用的技术基础；真实数据、跨平台 CI、复杂几何和性能发布门禁仍处于阶段性收口。
...
| 组件/产物 | 依赖 | 主要用途 | 当前状态 |
|---|---|---|---|
| `fast_gdb_linear` | 无 GDAL 运行时依赖 | 轻量部署、批量读取、纯 C++ 几何处理 | 阶段完成；跨平台发布门禁待收口 |
| `fast_gdb_hybrid` | GDAL | fast-gdb 主路径 + 曲线/复杂拓扑回退 | 阶段完成；Linux Hybrid CI 和真实对照待收口 |
...
| `explorgdb writer` | C++17；索引助手可选依赖 GDAL | 主数据表二进制写入和索引创建 | 阶段完成；系统表同步未完成 |

### 能力状态说明

- **已完成**：已有实现和对应自动化/本地证据支持。
- **阶段完成**：核心代码或部分真实数据已验证，但仍有发布门禁。
...
性能数据来自 **2026-06-16、macOS Apple Silicon、GDAL 3.9.3 OpenFileGDB 驱动**，数据模型为 Polygon + 4 个属性字段，规模覆盖 1K、10K、100K、1M 和 10M。以下数字是当前仓库基线，不代表所有机器或生产数据集的保证值。
...
性能优势主要来自 mmap、顺序访问、零拷贝 `FieldRef`/`string_view`、预分配、B+ 树按需导航、缓存和 FID 去重策略。完整表格、优化历程、失败实验和运行方法见 [性能基准与优化](../technical/01_性能基准与优化.md)。
...
| ASan/UBSan 几何专项 | 阶段完成 | 本机几何专项通过，Linux LSan 仍需 CI 证据 |
| 普通真实 FileGDB | 已完成 | release contract 已通过 |
| 两份真实曲线数据 | 阶段完成 | 结构、曲线状态、WKB-first 和部分 GDAL 对照已通过 |
| ArcGIS/GDAL 逐要素等价性 | 待验收 | Bezier 原生来源、曲线 Polygon 和面积/点包含对照仍缺证据 |
| Windows/Linux 纯 C++、Linux Hybrid CI | 待验收 | 需要当前提交的远端运行结果 |
| 非空 MultiPatch | 待验收 | 完整 part type 和表面拓扑尚未实现/验证 |
| 真实大规模性能基线 | 待验收 | 当前性能主要是合成数据基线 |
| Writer 系统表同步 | 待完成 | 主数据直写完成，生产级完整写入仍需继续完善 |

总体不能因为局部几何或自动化目标完成，就宣称正式发布验收完成。权威证据和后续补充位置见 [几何正确性与曲线支持实施报告](../planning/11_fast-gdb几何正确性与曲线支持实施报告.md) 和 [真实数据验收资料清单](../planning/13_fast-gdb真实数据验收资料清单.md)。

## 8. 后续路线

1. **短期**：补齐 ArcGIS 原生曲线、复杂 Polygon、FID 映射、Z/M/ZM 和真实性能数据。
2. **中期**：完成 Windows/Linux/macOS 与 Linux Hybrid 的当前提交 CI，补齐 ASan/UBSan/LSan 和发布门禁。
...
- 若本文与规划状态冲突，以 [规划文档状态索引](../planning/00_规划文档状态索引.md) 和最新实施报告为准。
