...
| [统一访问与 GDAL/S3 路由计划](planning/24_fast-gdb统一访问与GDAL_S3路由计划.md) | Local implementation in progress / release gates pending |
| [ADR-009：统一 FileGDB 访问与 GDAL/S3 路由](adr/ADR-009-unified-filegdb-routing.md) | Accepted：统一只读访问 seam；本地发布门禁待闭环；S3 仍 Experimental / Unverified |
...
| `fast_gdb::unified` | 统一 Dataset/Group/Layer/Cursor，Auto/FastOnly/GdalOnly |

统一访问入口是 v0.2.0 安装 target。它以 `Auto/FastOnly/GdalOnly` 选择后端：
本地 `.gdb` 默认使用 fast-gdb，`s3://`/`/vsis3/` 和 fast-gdb 不支持的只读能力使用
GDAL/OpenFileGDB。`FastFileGDB` 插件已实现；S3 路由尚缺真实 AWS 证据，因此仍为
...
该能力不引入 Writer、GDAL update wrapper、事务、发布层或 marker 写入能力。未采用 Adaptive 协调入口的调用方仍遵守 ADR-007 的关闭、写入、重开合同。
...
- S3 和其它对象存储在真实环境验收前不属于正式支持范围；统一 facade 和
  `FastFileGDB` 的本地只读能力属于 v0.2.0。
