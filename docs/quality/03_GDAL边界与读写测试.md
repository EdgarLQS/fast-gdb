...
> 本节对应已 Accepted 的 [ADR-008](../governance/adr/ADR-008-adaptive-reader-write-detection-gdal-fallback.md) 和[实施计划](../gdal/02_AdaptiveReader写入检测与GDAL回退计划.md)。当前实现验证同进程协调、Busy、过期、generation 诊断、fresh fallback 和本地 GDAL 写后重开；未知外部 Writer 与跨平台发布证据仍待补齐。
...
v0.2.0 统一入口把稳定本地 `.gdb` 路由到 fast-gdb，把 `s3://`/`/vsis3/` 和 fast-gdb
/

能力缺口路由到 GDAL OpenFileGDB。该路由不改变本测试定义的本地 Reader/GDAL 编辑
生命周期。S3 真实环境测试仍为 `SKIPPED`，不能声明 Supported。

详细接口、阶段和验收见 [统一访问与 GDAL/S3 路由计划](../gdal/03_fast-gdb统一访问与GDAL_S3路由计划.md)
和 [ADR-009](../governance/adr/ADR-009-unified-filegdb-routing.md)。
