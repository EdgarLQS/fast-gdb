...
For branch head `da5cabcc9b720fd287f9baa9a867f0123986a224`, GitHub created the Windows job and both POSIX jobs. The Windows job and the Linux/macOS jobs all completed before executing a step (`steps=None`) and exposed no job log URL. The repository's existing release, spatial-query, and geometry-correctness workflows exhibit the same pre-step failure pattern.
