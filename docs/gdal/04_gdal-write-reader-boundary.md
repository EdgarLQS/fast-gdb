...
或 GDAL 借用指针。详细设计见 [ADR-009](../governance/adr/ADR-009-unified-filegdb-routing.md)。
...
> 本节对应已 Accepted 的 [ADR-008](../governance/adr/ADR-008-adaptive-reader-write-detection-gdal-fallback.md)。同进程协调实现已进入可选 target；跨平台、压力、性能和多 GDAL 版本仍需独立发布验收。
