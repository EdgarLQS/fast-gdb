...
- **日期**：2026-07-23
- **当前正式合同**：[ADR-007：Reader-only 与 GDAL 编辑边界](../governance/adr/ADR-007-reader-only-gdal-edit-boundary.md)
- **决策依据**：[ADR-008：Adaptive Reader 写入检测与 fresh GDAL 只读回退](../governance/adr/ADR-008-adaptive-reader-write-detection-gdal-fallback.md)
/
